    ...
    <div className="space-y-8 max-w-7xl mx-auto transition-colors duration-300">
      <div className="flex flex-col space-y-2">
        <h2 className="text-2xl font-bold tracking-tight text-foreground flex items-center">
          <TrendingUp className="w-6 h-6 mr-3 text-emerald-icpc" />
          Executive Oversight Dashboard
        </h2>
        <p className="text-sm text-foreground/50">Intelligence overview and consolidated performance metrics for the current fiscal period.</p>
      </div>

      {/* Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat) => (
          <div key={stat.name} className="glass-effect p-6 rounded-sm relative overflow-hidden group hover:emerald-glow transition-all duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between px-2">
            <h3 className="text-xs font-mono text-foreground/40 uppercase tracking-[0.2em]">Inter-Agency Activity Flow</h3>
            <button className="text-[10px] text-emerald-icpc hover:text-emerald-icpc/80 font-bold uppercase transition-colors">View All Streams</button>
          </div>
          <div className="space-y-3">
            {recentActivity.map((activity) => (
              <div key={activity.id} className="glass-effect p-4 rounded-sm flex items-center justify-between group hover:bg-foreground/[0.02] transition-all">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-foreground/90 group-hover:text-foreground transition-colors">{activity.title}</h4>
                    <div className="flex items-center space-x-3 text-[10px] text-foreground/40 mt-1">
                      <span className="uppercase font-mono">{activity.agency}</span>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button className="p-1 px-2 text-[10px] font-bold uppercase text-foreground/40 hover:text-foreground hover:bg-foreground/10 rounded-sm transition-all">
                    Details
                  </button>
                </div>
              </div>
        ...
        <div className="space-y-4">
          <div className="px-2">
            <h3 className="text-xs font-mono text-foreground/40 uppercase tracking-[0.2em]">System Compliance</h3>
          </div>
          <div className="glass-effect p-6 rounded-sm border-t-2 border-emerald-icpc">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="w-12 h-12 rounded-full border-4 border-emerald-icpc/20 border-t-emerald-icpc animate-[spin_3s_linear_infinite]"></div>
            </div>
            <div className="space-y-4">
